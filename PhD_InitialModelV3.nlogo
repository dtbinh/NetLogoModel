...
;  set-default-shape Locschools "house"
  ...
  ; set a starting time
  set cur_time time:create "2015/01/01 00:00"
  ...
  foreach population [
    ...
    ask one-of patches with [pcolor = item 1 currentperson] [
      sprout-people 1 [
        ...
        set schedule setup_activity_schedule counterMobile
...
to-report setup_activity_schedule [pid]
  let data []
  let dict table:make
  set data csv:from-file (word "ActivitySchedule_" pid ".csv")
...
to go
  ...
  ask people [
    ...
    set loc table:get schedule (ticks)
    ;print loc
